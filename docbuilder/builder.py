...
def _shade_cell(cell, color: RGBColor) -> None:
    ...
    shading_elem = shading.makeelement(qn("w:shd"), {
        qn("w:val"): "clear",
        qn("w:color"): "auto",
        qn("w:fill"): f"{color:02X}{color:02X}{color:02X}"[:6],
    })
...
def _apply_bold_inline(paragraph) -> None:
    """Apply bold to inline markers in an existing paragraph."""
    for run in paragraph.runs:
